...
-(void)parserExcel_XLS_WithPath:(NSString *)filePath {
    ...
    NSMutableDictionary *language = [NSMutableDictionary dictionary];
    NSMutableDictionary *languageDetail = [NSMutableDictionary dictionary];
    NSMutableDictionary *languageNumber = [NSMutableDictionary dictionary];
    ...
    int rows = reader.getRows + 1;
    int cols = reader.getCols;
    for(int r=1;r<=cols;r++) {
        for(int c=1;c<=rows;c++) {
            unichar ch =64 + c;
            NSString *str =[NSString stringWithUTF8String:(char *)&ch];
            DHcell *cell = [reader cellInWorkSheetIndex:0 row:c col:r];
            ZContent *content = [[ZContent alloc] init];
            content.sheetName = [reader sheetNameAtIndex:0];
            content.keyName = [NSString stringWithFormat: @"%@", str];
            content.value = [cell dump];
            [resultArray addObject:content];
            
            if (r == 1) {
                [language setValue:content.value forKey:[NSString stringWithFormat:@"%d",c]];
            } else {
                if (c == 1) {
                    [languageDetail setValue:[NSMutableDictionary dictionary] forKey:content.value];
                    [languageNumber setValue:content.value forKey:[NSString stringWithFormat:@"%d",r]];
                } else {
                    NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithDictionary:[languageDetail objectForKey:[languageNumber objectForKey:[NSString stringWithFormat:@"%d",r]]]];
                    NSString *key = [NSString stringWithFormat:@"\"%@\"",[language objectForKey:[NSString stringWithFormat:@"%d",c]]];
                    NSString *value = [NSString stringWithFormat:@"\"%@\"",content.value];
                    [dic setValue:value forKey:key];
                    [languageDetail setValue:dic forKey:[languageNumber objectForKey:[NSString stringWithFormat:@"%d",r]]];
                }
            }
        }
    }
    ...
    if([_delegate respondsToSelector:@selector(parser:success:)]){
        ...
        [weakSelf.delegate parser:weakSelf success:languageDetail];
    }else{
